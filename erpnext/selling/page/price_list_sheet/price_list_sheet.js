...
jQuery.fn.getpopup = function() {
		...
		$.each(jsonobj, function(idx, obj) {
			...
			data=data+'<tr class="item-row" data-row="'+k+'"><td width="20px" data-row="'+k+'" data-col="0"><a href="javascript:;" class="del" onClick="myTable_DelRow('+k+')">X</a></td><td data-row="'+k+'" data-col="1"><div class="autocomplete"><input onClick="autocomplete1(\'price_list_data'+k+'\')"  type="text" class="price_list_data" id="price_list_data'+k+'" name="price_list_data" value="'+obj["price_list"]+'"></div></td></tr>';
			k = k+1;
		});		

		data=data+'</table><button onClick="AddNewRow1()" class="btn-primary" id="addnewRow">Add Row</button><script type="text/javascript">$("#qtyTable1").on("click", ".price_list_data", function() { var row = $(this).parents(".item-row"); var idd = row.find(".price_list_data").attr("id"); autocomplete1(idd); });</script>';
		var fields = [
...
frappe.price_list_sheet= {
	...
	run: function(page,obj) {
		...
		frappe.call({
			...
			me.body=$('<table id="qtyTable"  border="1" class="display nowrap dataTable dtr-inline" width="100%"></table><script type="text/javascript">$("#qtyTable").css("cursor","pointer"); $("#qtyTable").on("click","td",function(){ $(this).getpopup1(); });</script>').appendTo(me.page.main);
		...
		{
			me.body=$('<table id="qtyTable"  border="1" class="display nowrap dataTable dtr-inline" width="100%"></table><script type="text/javascript">$("#qtyTable").css("cursor","pointer");$(".first").click(function(){alert("Test");	}); $("#qtyTable").on("click","td",function(){ $(this).getpopup1(); });</script>').appendTo(me.page.main);
